...
import { PACKAGING_IMAGES } from "../config/images";
...
export default function PackagingSection() {
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      setCurrentIndex((prev) => (prev + 1) % PACKAGING_IMAGES.length);
    }, 4000);
          ...
          <div className="order-1 md:order-2">
            <div className="relative rounded-3xl overflow-hidden aspect-[4/3] bg-purple-950/30">
              <AnimatePresence mode="wait">
                <motion.img
                  key={currentIndex}
                  src={PACKAGING_IMAGES[currentIndex]}
